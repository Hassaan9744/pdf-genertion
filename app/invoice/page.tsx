...
import invoiceSchema from "@/invoiceSchema.json";
import type { Font } from "@pdfme/common";
import { generate } from "@pdfme/generator";
import { getFontsData } from "@/lib/utilis";

// Define font URLs or Base64 encoded data for Roboto fonts
...
const Page = () => {
  ...
  // Add a dollar sign to each item's price
  ...
  // Calculate the total
  // const total = formattedTableData.reduce(
  //   (sum, item) => sum + parseFloat(item[1].replace("$", "")),
  //   0
  // );

  // Add the total row with a dollar sign
  // formattedTableData.push(["Total", `$${total.toFixed(2)}`]);
  ...
  const inputs = [
    {
      ...
      company_address: "2938 Whitetail Lane , Dallas ,Texas",
